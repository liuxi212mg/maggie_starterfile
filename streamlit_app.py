...
# Display a header
st.header("Custom Dropdown with Hoverable Full Content")

# CSS for the custom dropdown and hover effect
st.markdown("""
<style>
...
# Display the custom dropdown using HTML and CSS
dropdown_html = '<div class="custom-dropdown"><select id="dropdown">'
for i, label in enumerate(labels):
    dropdown_html += f'<option value="{i}">{label}</option>'
dropdown_html += '</select></div>'

# Inject the dropdown HTML into the Streamlit app
st.markdown(dropdown_html, unsafe_allow_html=True)

# Get user selection based on the index selected
selected_index = st.selectbox("Select an option", options=labels)

# Show the description for the selected option
st.write(f"You selected: {selected_index}")
st.write(f"Description: {descriptions[selected_index]}")
